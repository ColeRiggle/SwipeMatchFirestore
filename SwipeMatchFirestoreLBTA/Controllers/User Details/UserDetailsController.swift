...
//
//  Created by Cole Riggle on 10/22/20.
...
class UserDetailsController: UIViewController, UIScrollViewDelegate {
    ...
    let swipingPhotosController = SwipingPhototsController(isCardViewMode: false)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if (width > view.frame.width) {
            let swipingView = swipingPhotosController.view!
            swipingView.frame = CGRect(x: -changeY, y: -changeY, width: width, height: width + extraSwipingHeight)
        }
    }
